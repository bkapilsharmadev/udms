...
module.exports.excelBufferToJSON = async (buffer) => {
  try {
    ...
    const headerRow = worksheet
      .getRow(1)
      .values.slice(1)
      .map((header) => (typeof header === "string" ? header.trim() : header)); // Skip the first cell (0 index is null)
    ...
    worksheet.eachRow((row, rowNumber) => {
      ...
      headerRow.forEach((header, colIndex) => {
        let cellValue = row.getCell(colIndex + 1).value; // Adjust index for 1-based column indexing
        if (typeof cellValue === "string") {
          cellValue = cellValue.trim(); // Trim data cells if they are strings
        }
...
module.exports.createExcelFile = async (
  headers,
  dropDownOptions,
...
module.exports.areFieldsUnique = (arr1, arr2, fieldMapping) => {
  // Function to Check if the some field value of two array of objects are same
  for (const [sourceField, targetField] of Object.entries(fieldMapping)) {
    const sourceValues = arr1.map((item) => item[sourceField]);
    const targetValues = arr2.map((item) => item[targetField]);

    if (sourceValues.some((value) => targetValues.includes(value))) {
      return false;
    }
  }
