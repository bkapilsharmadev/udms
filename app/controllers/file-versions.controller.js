...
module.exports.updateFileVersion = async (req, res, next) => {
	...
	const result = await fileVersionService.updateFileVersion({
		version_id,
		hash,
		document_url,
...
module.exports.getFileVersionsByDocumentId = async (req, res, next) => {
	...
	const result = await fileVersionService.getFileVersionsByDocumentId(
		document_id
	);
	res.status(200).json(result);
};
...
module.exports.downloadFile = async (req, res, next) => {
	...
	const file = await fileVersionService.downloadFile(version_id);

    console.log("file>>>> ", file);

    res.download(file.filePath, file.originalFileName, (err) => {
        if (err) {
